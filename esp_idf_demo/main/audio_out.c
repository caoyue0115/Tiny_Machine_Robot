...
static void audio_stream_task(void *arg)
{
    ...
    int64_t rebuffer_start_us = 0;

    while (true) {
        if (!prebuffer_done || rebuffering) {
            const size_t target_bytes = rebuffering
                                            ? DEMO_REALTIME_AUDIO_JITTER_REBUFFER_BYTES
                                            : DEMO_REALTIME_AUDIO_JITTER_PREBUFFER_BYTES;
            ...
            if (lock_ret == ESP_OK) {
                level = audio_jitter_level_locked();
                total_in = s_audio_out_state.jitter_total_in;
                total_out = s_audio_out_state.jitter_total_out;
                stream_started = s_audio_out_state.stream_task_started;
                ready = level >= target_bytes || !stream_started;
                should_exit = !stream_started && level == 0;
                audio_out_unlock();
            }
            if (should_exit) {
                break;
            ...
            if (!ready) {
                ...
                if (rebuffering && audio_out_lock() == ESP_OK) {
                    if (s_audio_out_state.jitter_playback_started) {
                        s_audio_out_state.jitter_underrun_count++;
                        s_audio_out_state.jitter_underrun_us += esp_timer_get_time() - wait_start_us;
            ...
            if (rebuffering) {
                ESP_LOGI(TAG,
                         "Realtime jitter rebuffer resume level=%u target=%u wait_ms=%.1f total_in=%u total_out=%u stream_active=%d",
                         (unsigned)level,
                         (unsigned)target_bytes,
                         rebuffer_start_us > 0
                             ? (double)(esp_timer_get_time() - rebuffer_start_us) / 1000.0
                             : 0.0,
                         (unsigned)total_in,
                         (unsigned)total_out,
                         stream_started ? 1 : 0);
        ...
        if (item == NULL) {
            ...
            if (lock_ret == ESP_OK) {
                ...
                total_in = s_audio_out_state.jitter_total_in;
                total_out = s_audio_out_state.jitter_total_out;
                should_exit = !s_audio_out_state.stream_task_started && level == 0;
                if (!should_exit && s_audio_out_state.jitter_playback_started) {
                    s_audio_out_state.jitter_underrun_count++;
                    s_audio_out_state.jitter_underrun_us += receive_elapsed_us;
                    audio_jitter_update_level_stats_locked(0);
                    enter_rebuffer = s_audio_out_state.stream_task_started && !rebuffering;
                }
                audio_out_unlock();
            }
            if (should_exit) {
            ...
            if (enter_rebuffer) {
                rebuffering = true;
                rebuffer_start_us = esp_timer_get_time();
                ESP_LOGW(TAG,
                         "Realtime jitter underrun entering rebuffer level=%u target=%u total_in=%u total_out=%u",
                         (unsigned)level,
                         (unsigned)DEMO_REALTIME_AUDIO_JITTER_REBUFFER_BYTES,
                         (unsigned)total_in,
                         (unsigned)total_out);
            }
            continue;
                                             ...
                                             size_t *written_bytes)
{
    ...
    if (xRingbufferSend(s_audio_out_state.jitter_ringbuf,
        ...
        if (ret == ESP_OK) {
            ...
            total_out = s_audio_out_state.jitter_total_out;
            level = audio_jitter_level_locked();
            playback_started = s_audio_out_state.jitter_playback_started;
            audio_out_unlock();
        }
        ESP_LOGE(TAG,
                 "Realtime jitter enqueue timeout chunk_bytes=%u wait_ms=%d level=%u buffer_bytes=%u total_in=%u total_out=%u playback_started=%d",
                 (unsigned)pcm_bytes_size,
                 DEMO_AUDIO_PLAY_WRITE_TIMEOUT_MS,
                 (unsigned)level,
                 (unsigned)DEMO_REALTIME_AUDIO_JITTER_BUFFER_BYTES,
                 (unsigned)total_in,
                 (unsigned)total_out,
    ...
    if (send_elapsed_us > (int64_t)DEMO_REALTIME_AUDIO_ENQUEUE_WARN_MS * 1000) {
        ESP_LOGW(TAG,
                 "Realtime jitter enqueue backpressure chunk_bytes=%u wait_ms=%.1f",
                 (unsigned)pcm_bytes_size,
                 (double)send_elapsed_us / 1000.0);
    }
