...
static bool cloud_is_transient_audio_read_result(int read_len)
{
    return read_len == -ESP_ERR_HTTP_EAGAIN;
}
...
static void cloud_log_realtime_audio_stall_diag(esp_http_client_handle_t client,
                                                const cloud_stream_runtime_t *runtime,
                                                const cloud_realtime_audio_metrics_t *metrics,
                                                int64_t no_data_us,
                                                int read_len,
                                                bool saw_first_chunk,
                                                const char *phase)
{
    int socket_errno = client != NULL ? esp_http_client_get_errno(client) : 0;
    bool complete = client != NULL ? esp_http_client_is_complete_data_received(client) : false;
    ...
    UBaseType_t encoded_waiting = 0;
    UBaseType_t pcm_waiting = 0;
    if (runtime != NULL) {
        if (runtime->encoded_queue != NULL) {
            encoded_waiting = uxQueueMessagesWaiting(runtime->encoded_queue);
        }
        if (runtime->pcm_queue != NULL) {
            pcm_waiting = uxQueueMessagesWaiting(runtime->pcm_queue);
        }
    }

    ESP_LOGW(TAG,
             "Realtime audio stall_diag phase=%s no_data_ms=%.1f read_len=%d socket_errno=%d(%s) http_complete=%d saw_first_chunk=%d http_status=%d stream_bytes=%u chunk_count=%u packet_count=%u max_gap_ms=%.1f read_stall_count=%u encoded_q=%u pcm_q=%u rssi=%d channel=%u wifi_auth=%d wifi_ps=%d ap_info=%s ps_info=%s free_8bit=%u largest_8bit=%u free_spiram=%u largest_spiram=%u",
             phase != NULL ? phase : "unknown",
             (double)no_data_us / 1000.0,
             read_len,
             socket_errno,
             socket_errno != 0 ? strerror(socket_errno) : "none",
             complete ? 1 : 0,
             saw_first_chunk ? 1 : 0,
             metrics != NULL ? metrics->http_status : 0,
             ...
             metrics != NULL ? (double)metrics->max_inter_chunk_gap_us / 1000.0 : 0.0,
             metrics != NULL ? (unsigned)metrics->read_stall_count : 0,
             (unsigned)encoded_waiting,
             (unsigned)pcm_waiting,
             ap_ret == ESP_OK ? (int)ap.rssi : 0,
             ap_ret == ESP_OK ? (unsigned)ap.primary : 0,
             ap_ret == ESP_OK ? (int)ap.authmode : -1,
                                             ...
                                             cloud_realtime_audio_metrics_t *metrics)
{
    ...
    esp_http_client_config_t config = {
        ...
        /* [B] 下行音频抗抖动:开启 TCP keepalive。答复中途若链路短暂卡顿,
         * keepalive 探测能维持连接、更快识别真死链,避免半句被判失败。
         * 仅缓解“卡一下又恢复”,治本仍靠 [A] 稳链路。 */
        .keep_alive_enable = true,
        .keep_alive_idle = DEMO_REALTIME_AUDIO_TCP_KEEPALIVE_IDLE_S,
        .keep_alive_interval = DEMO_REALTIME_AUDIO_TCP_KEEPALIVE_INTERVAL_S,
        .keep_alive_count = DEMO_REALTIME_AUDIO_TCP_KEEPALIVE_COUNT,
    };
    esp_http_client_handle_t client = esp_http_client_init(&config);
    if (client == NULL) {
        free(audio_headers);
    ...
    ret = esp_http_client_set_timeout_ms(client, DEMO_REALTIME_AUDIO_READ_POLL_TIMEOUT_MS);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG,
                 "Failed to set realtime audio read timeout poll_ms=%d err=%s",
                 DEMO_REALTIME_AUDIO_READ_POLL_TIMEOUT_MS,
                 esp_err_to_name(ret));
        esp_http_client_close(client);
        esp_http_client_cleanup(client);
        free(audio_headers);
    ...
    int64_t last_read_progress_us = esp_timer_get_time();
    int64_t last_read_stall_log_us = 0;
    ...
    while (true) {
        ...
        if (read_len < 0 && cloud_is_transient_audio_read_result(read_len)) {
            const int64_t no_data_us = read_done_us - last_read_progress_us;
            if (metrics != NULL) {
                metrics->read_stall_count++;
                if (no_data_us > metrics->max_read_stall_us) {
                    metrics->max_read_stall_us = no_data_us;
                }
            }
            if (no_data_us >= ((int64_t)DEMO_REALTIME_AUDIO_READ_TIMEOUT_MS * 1000)) {
                ESP_LOGE(TAG,
                         "Realtime audio read timeout no_data_ms=%.1f max_no_data_ms=%d poll_timeout_ms=%d chunks=%u bytes=%u",
                         (double)no_data_us / 1000.0,
                         DEMO_REALTIME_AUDIO_READ_TIMEOUT_MS,
                         DEMO_REALTIME_AUDIO_READ_POLL_TIMEOUT_MS,
                         metrics != NULL ? (unsigned)metrics->chunk_count : 0,
                         metrics != NULL ? (unsigned)metrics->total_audio_bytes : 0);
                cloud_log_realtime_audio_stall_diag(client,
                                                    &runtime,
                                                    metrics,
                                                    no_data_us,
                                                    read_len,
            ...
            if (no_data_us >= ((int64_t)DEMO_REALTIME_AUDIO_STALL_DIAG_MS * 1000) &&
                (last_read_stall_log_us == 0 ||
                 read_done_us - last_read_stall_log_us >=
                     ((int64_t)DEMO_REALTIME_AUDIO_STALL_DIAG_PERIOD_MS * 1000))) {
                cloud_log_realtime_audio_stall_diag(client,
                                                    &runtime,
                                                    metrics,
                                                    no_data_us,
                                                    ...
                                                    saw_first_chunk,
                                                    "eagain");
                last_read_stall_log_us = read_done_us;
            }
            vTaskDelay(pdMS_TO_TICKS(10));
            continue;
        }
        if (read_len < 0) {
            cloud_log_realtime_audio_stall_diag(client,
                                                &runtime,
                                                metrics,
                                                read_done_us - last_read_progress_us,
                                                read_len,
                                                saw_first_chunk,
                                                "read_fail");
        ...
        if (read_len == 0) {
            ...
            if (no_data_us >= ((int64_t)DEMO_REALTIME_AUDIO_READ_TIMEOUT_MS * 1000)) {
                ESP_LOGE(TAG,
                         "Realtime audio read returned no data for %.1f ms max_no_data_ms=%d",
                         (double)no_data_us / 1000.0,
                         DEMO_REALTIME_AUDIO_READ_TIMEOUT_MS);
                cloud_log_realtime_audio_stall_diag(client,
                                                    &runtime,
        ...
        if (last_read_stall_log_us != 0) {
            const int socket_errno = esp_http_client_get_errno(client);
            ESP_LOGI(TAG,
                     "Realtime audio read recovered no_data_ms=%.1f read_len=%d socket_errno=%d(%s)",
                     (double)(read_done_us - last_read_progress_us) / 1000.0,
                     read_len,
                     socket_errno,
                     socket_errno != 0 ? strerror(socket_errno) : "none");
            last_read_stall_log_us = 0;
        }
        last_read_progress_us = read_done_us;
